    ...
    private Price convertResponseToPrice(final String[] row) {
        if (row.length == PriceConverter.values().length) {
            return Price.builder()
                    ...
                    .priceDayOverDay(
                            getIntegerFormat(row[PriceConverter.PRICE_DAY_OVER_DAY.getIndex()]))
                    ...
                    .volumePercentage(
                            getBigDecimalFormat(row[PriceConverter.VOLUME_PERCENTAGE.getIndex()]))
                    ...
                    .priceHighLimit(
                            getIntegerFormat(row[PriceConverter.PRICE_HIGH_LIMIT.getIndex()]))
                    .dateOfHighPrice(
                            getFormattedDate(
                                    row[PriceConverter.DATE_OF_HIGH_PRICE.getIndex()],
                                    DATE_OF_PRICE_FORMAT))
                    .yearlyHigh(getIntegerFormat(row[PriceConverter.YEARLY_HIGH.getIndex()]))
                    .rateOfDeviationOfYearlyHigh(
                            getBigDecimalFormat(row[PriceConverter.RATE_OF_DEVIATION_OF_YEARLY_HIGH
                                    .getIndex()]))
                    .dateOfLowPrice(
                            getFormattedDate(
